...
def loss_function(real, pred, **kwargs):
    if attention_mode == HARD_ATTENTION:
        decoder = kwargs['decoder']
        attention_weights, attention_location = decoder.attention_weights, decoder.attention_location
        loss, gt_likelihood = decoder.attention.loss(real, pred, attention_weights, attention_location)
        decoder.gt_likelihoods.append(gt_likelihood)  # Needed to keep track of for updating running average b
        return loss
    else:
...
def train_step(img_tensor, target, decoder, encoder, tokenizer, optimizer, flag):
    ...
    with tf.GradientTape() as tape:
        ...
        for i in range(1, target.shape[1]):
            ...
            predictions, hidden, _ = decoder(dec_input, features, hidden, flag)

            if attention_mode == HARD_ATTENTION:
                loss += loss_function(target[:, i], predictions, decoder=decoder)
            else:
    ...
    if attention_mode == HARD_ATTENTION:
        decoder.attention.update(decoder.gt_likelihoods, target.shape[1])
